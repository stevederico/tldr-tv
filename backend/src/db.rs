...
impl Row {
    ...
    /// Read a column as `f64`. Integers coerce, matching SQLite's affinity.
    pub fn real(&self, col: &str) -> Option<f64> {
        match self.get(col) {
...
const INDEXES: &[&str] = &[
    ...
    "CREATE INDEX IF NOT EXISTS idx_guides_author ON Guides(author)",
];
...
/// Columns an older `Guides` table may be missing.
///
/// `chapters_json` is omitted: `ADD COLUMN` cannot attach a `NOT NULL` column
/// without a default, and every historical `Guides` table already has it.
const GUIDE_COLUMNS: &[(&str, &str)] = &[
    ("author", "TEXT"),
    ("date", "TEXT"),
    ("duration", "INTEGER"),
    ("audio_url", "TEXT"),
...
fn ensure_schema(db: &Db) -> Result<(), DbError> {
    ...
    add_missing_columns(db, "Guides", GUIDE_COLUMNS)?;
...
impl Pool {
    ...
    /// Public catalog rows, newest first. Transcript and timings are omitted.
    ///
    /// @returns Summary rows. `chapters_json` is included so callers can count chapters.
    pub fn list_public_guides(&self) -> Result<Vec<GuideRecord>, DbError> {
        self.with(|db| {
            let rows = db.query(
                "SELECT slug, title, author, date, duration, thumbnail, chapters_json,
                        jobs_json, visibility, created_by, created_at, updated_at
                 FROM Guides WHERE visibility = 'public' ORDER BY created_at DESC",
                &[],
            )?;
            Ok(rows.iter().map(guide_from_row).collect())
        })
    }

    /// One guide by slug, or `None` when the row is absent.
    pub fn get_guide(&self, slug: &str) -> Result<Option<GuideRecord>, DbError> {
        self.with(|db| {
            let rows = db.query("SELECT * FROM Guides WHERE slug = ?", &[Value::Text(slug.into())])?;
            Ok(rows.first().map(guide_from_row))
        })
    }

    /// Insert a new guide. `created_by` is the caller when the request is authenticated.
    ///
    /// # Errors
    /// Duplicate `slug` surfaces as a SQLite unique-constraint error.
    pub fn insert_guide(&self, guide: &GuideRecord) -> Result<(), DbError> {
        self.with(|db| {
            db.run(
                "INSERT INTO Guides
                  (slug, title, author, date, duration, audio_url, thumbnail,
                   ...
                   chapters_json, timing_json, visibility, created_by, created_at, updated_at)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                &guide_insert_params(guide),
            )?;
            Ok(())
        })
    }

    /// Replace mutable columns. The `WHERE` includes the caller so one user cannot
    /// overwrite another user's row. Unowned rows stay editable.
    ///
    /// @param actor - Authenticated user id, or `None` for an anonymous caller
    /// @returns Rows updated (0 when the slug is missing or owned by someone else)
    pub fn update_guide(&self, guide: &GuideRecord, actor: Option<&str>) -> Result<u64, DbError> {
        self.with(|db| {
            let (sql, actor_id) = match actor {
                Some(id) => (
                    "UPDATE Guides SET
                      title=?, author=?, date=?, duration=?, audio_url=?, thumbnail=?,
                      timing_offset=?, default_view_mode=?, transcript=?, summary=?, source_url=?,
                      jobs_json=?, chapters_json=?, timing_json=?, visibility=?, updated_at=?
                     WHERE slug=? AND (created_by = ? OR created_by IS NULL)",
                    Some(id),
                ),
                None => (
                    "UPDATE Guides SET
                      ...
                      timing_offset=?, default_view_mode=?, transcript=?, summary=?, source_url=?,
                      jobs_json=?, chapters_json=?, timing_json=?, visibility=?, updated_at=?
                     WHERE slug=? AND created_by IS NULL",
                    None,
                ),
            };
            let mut params = guide_update_params(guide);
            if let Some(id) = actor_id {
                params.push(Value::Text(id.into()));
            }
            Ok(db.run(sql, &params)?.changes.max(0) as u64)
        })
    }

    /// Write a guide the server already authorized, keyed only by slug.
    ///
    /// Background jobs use this. HTTP handlers must use [`Pool::update_guide`]
    /// so the caller id stays in the `WHERE` clause.
    pub fn update_guide_by_slug(&self, guide: &GuideRecord) -> Result<u64, DbError> {
        self.with(|db| {
            Ok(db
                .run(
                    "UPDATE Guides SET
    ...
    /// Delete one guide, scoped the same way as [`Pool::update_guide`].
    ///
    /// @returns Rows deleted
    pub fn delete_guide(&self, slug: &str, actor: Option<&str>) -> Result<u64, DbError> {
        self.with(|db| {
            let (sql, params) = match actor {
                Some(id) => (
                    "DELETE FROM Guides WHERE slug = ? AND (created_by = ? OR created_by IS NULL)",
                    vec![Value::Text(slug.into()), Value::Text(id.into())],
                ),
                None => (
                    "DELETE FROM Guides WHERE slug = ? AND created_by IS NULL",
                    vec![Value::Text(slug.into())],
                ),
            };
            Ok(db.run(sql, &params)?.changes.max(0) as u64)
        })
...
/// One `Guides` row. JSON columns stay text; callers parse them.
#[derive(Debug, Clone, PartialEq)]
pub struct GuideRecord {
    ...
    /// Publication date string, as scraped or typed.
    pub date: Option<String>,
    /// Duration in seconds.
    pub duration: Option<i64>,
    /// Public audio URL, usually `/audio/<slug>.mp3`.
    pub audio_url: Option<String>,
    /// Cover image URL.
    pub thumbnail: Option<String>,
    /// Seconds added to every word timestamp on the client.
    ...
    /// Page the essay was fetched from.
    pub source_url: Option<String>,
    /// JSON object of pipeline job states.
    pub jobs_json: Option<String>,
    /// JSON array of chapters. Never null in the table.
    pub chapters_json: String,
    /// JSON `{words:[{w,t}]}`.
    pub timing_json: Option<String>,
    /// `public` or `private`.
    pub visibility: String,
    /// `Users._id` of the creator. `None` for seeded catalog rows.
...
/// Map a selected row. Missing columns read as empty, so the list query can omit them.
fn guide_from_row(row: &Row) -> GuideRecord {
    GuideRecord {
        slug: row.text("slug").unwrap_or("").to_string(),
        title: row.text("title").unwrap_or("").to_string(),
        author: row.text("author").map(str::to_string),
        date: row.text("date").map(str::to_string),
        duration: row.int("duration").or_else(|| row.real("duration").map(|n| n as i64)),
        audio_url: row.text("audio_url").map(str::to_string),
        thumbnail: row.text("thumbnail").map(str::to_string),
        timing_offset: row.real("timing_offset").unwrap_or(0.0),
        default_view_mode: row.text("default_view_mode").unwrap_or("real").to_string(),
        transcript: row.text("transcript").map(str::to_string),
        summary: row.text("summary").map(str::to_string),
        source_url: row.text("source_url").map(str::to_string),
        jobs_json: row.text("jobs_json").map(str::to_string),
        chapters_json: row.text("chapters_json").unwrap_or("[]").to_string(),
        timing_json: row.text("timing_json").map(str::to_string),
        visibility: row.text("visibility").unwrap_or("public").to_string(),
        created_by: row.text("created_by").map(str::to_string),
        created_at: row.int("created_at").unwrap_or(0),
        updated_at: row.int("updated_at").unwrap_or(0),
    }
}

/// Insert parameters, in column order.
fn guide_insert_params(guide: &GuideRecord) -> Vec<Value> {
    vec![
        Value::Text(guide.slug.clone()),
        Value::Text(guide.title.clone()),
        opt_text(guide.author.as_deref()),
        opt_text(guide.date.as_deref()),
        match guide.duration {
            Some(n) => Value::Int(n),
            None => Value::Null,
        },
        opt_text(guide.audio_url.as_deref()),
        opt_text(guide.thumbnail.as_deref()),
        Value::Real(guide.timing_offset),
        Value::Text(guide.default_view_mode.clone()),
        opt_text(guide.transcript.as_deref()),
        opt_text(guide.summary.as_deref()),
        opt_text(guide.source_url.as_deref()),
        opt_text(guide.jobs_json.as_deref()),
        Value::Text(guide.chapters_json.clone()),
        opt_text(guide.timing_json.as_deref()),
        Value::Text(guide.visibility.clone()),
        opt_text(guide.created_by.as_deref()),
        Value::Int(guide.created_at),
        Value::Int(guide.updated_at),
    ]
}

/// SET parameters plus the slug placeholder.
fn guide_update_params(guide: &GuideRecord) -> Vec<Value> {
    vec![
        Value::Text(guide.title.clone()),
        opt_text(guide.author.as_deref()),
        opt_text(guide.date.as_deref()),
...
/// `NULL` for a missing string, otherwise the text.
...
mod tests {
    ...
    fn opens_pool_and_creates_schema() {
        ...
        assert_eq!(names, vec!["Auths", "Guides", "Users", "WebhookEvents"]);
    }

    #[test]
    fn guide_delete_is_scoped_to_the_caller() {
        let tmp = TempDb::new("guides");
        let pool = Pool::open(&tmp.path(), 1).expect("pool");
        let now = 1_700_000_000_000;
        let mut owned = sample_guide("owned-one", Some("user-a".into()));
        owned.created_at = now;
        owned.updated_at = now;
        pool.insert_guide(&owned).unwrap();
        let mut seeded = sample_guide("seeded", None);
        seeded.created_at = now;
        seeded.updated_at = now;
        pool.insert_guide(&seeded).unwrap();

        assert_eq!(pool.delete_guide("owned-one", None).unwrap(), 0);
        assert!(pool.get_guide("owned-one").unwrap().is_some());
        assert_eq!(pool.delete_guide("owned-one", Some("user-b")).unwrap(), 0);
        assert_eq!(pool.delete_guide("owned-one", Some("user-a")).unwrap(), 1);
        assert_eq!(pool.delete_guide("seeded", None).unwrap(), 1);
    }

    /// A guide row with the fields the catalog tests care about.
    fn sample_guide(slug: &str, owner: Option<String>) -> GuideRecord {
        GuideRecord {
            slug: slug.into(),
            title: "Title".into(),
            author: None,
            ...
            transcript: Some("Hello transcript text for the test guide.".into()),
            summary: None,
            source_url: None,
            jobs_json: None,
            chapters_json: "[]".into(),
