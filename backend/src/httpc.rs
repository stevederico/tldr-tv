...
//! `stripe.rs` are private to that module. Guide fetch, xAI, and Unsplash
//! need their own client. Zero-crate: libcurl is linked as a system library
//! (`#[link(name = "curl")]`), exactly as `stripe.rs` links it, and every
//! `unsafe` block below carries a `// SAFETY:` justification. The exported
//! surface ([`get`], [`get_follow`], [`post_json`], [`post_form`], [`HttpResponse`]) is safe.
//!
//! API calls do not follow redirects, so an `Authorization` header cannot be
//! replayed onto another host. [`get_follow`] is only for public page and
//! image fetches that send no credentials.
...
pub fn get(url: &str, headers: &[(&str, &str)], timeout_ms: i64) -> Result<HttpResponse, HttpError> {
    request(Method::Get, url, headers, None, timeout_ms, false)
}

/// `GET url`, following up to five http(s) redirects.
///
/// Do not send credentials with this. Redirects are for public pages and
/// image CDNs only.
pub fn get_follow(url: &str, headers: &[(&str, &str)], timeout_ms: i64) -> Result<HttpResponse, HttpError> {
    request(Method::Get, url, headers, None, timeout_ms, true)
}
...
pub fn post_json(
    ...
    request(Method::Post, url, &all, Some(body.as_bytes()), timeout_ms, false)
}
...
fn request(
    ...
    let setup = unsafe {
        ...
        let redirects: c_long = if follow { 1 } else { 0 };
        rc |= curl_easy_setopt(h, CURLOPT_FOLLOWLOCATION, redirects);
        rc |= curl_easy_setopt(h, CURLOPT_MAXREDIRS, 5 as c_long);
