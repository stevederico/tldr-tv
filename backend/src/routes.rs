...
fn dispatch(state: &AppState, req: &Request) -> Response {
    match (req.method.as_str(), req.path.as_str()) {
        ...
        // Literal paths before /api/guides/:slug. `stream.mp3` and the other
        // suffixes are matched inside `guides::by_slug`, not as slugs.
        ("POST", "/api/fetch-url") => crate::guides::fetch_url(state, req),
        ("POST", "/api/tts") => crate::guides::tts_once(state, req),
        ("GET", "/api/guides") => crate::guides::list_guides(state),
        ("POST", "/api/guides") => crate::guides::create_guide(state, req),
        (_, p) if p.starts_with("/api/guides/") => crate::guides::by_slug(state, req),
        ("GET" | "HEAD", p) if p.starts_with("/audio/") || p.starts_with("/images/") => {
            crate::guides::serve_media(state, req)
        }
...
/// could be issued.
pub(crate) fn require_csrf(state: &AppState, req: &Request, user_id: &str) -> Result<(), Response> {
...
mod tests {
    ...
    #[test]
    fn guides_create_list_and_delete_round_trip() {
        let (state, dir) = test_state();
        let mut req = Request::for_test("POST", "/api/guides");
        req.set_test_body(br#"{"title":"Founder Mode","transcript":"A long enough transcript for the catalog."}"#.to_vec());
        let created = handle(&state, req);
        assert_eq!(created.status, 201);
        let slug = json_body(&created).get_str("slug").unwrap().to_string();
        assert_eq!(slug, "founder-mode");

        let listed = handle(&state, Request::for_test("GET", "/api/guides"));
        assert_eq!(listed.status, 200);
        let one = handle(&state, Request::for_test("GET", &format!("/api/guides/{slug}")));
        assert_eq!(one.status, 200);
        assert_eq!(json_body(&one).get_str("title"), Some("Founder Mode"));

        let deleted = handle(&state, Request::for_test("DELETE", &format!("/api/guides/{slug}")));
        assert_eq!(deleted.status, 200);
        let gone = handle(&state, Request::for_test("GET", &format!("/api/guides/{slug}")));
        assert_eq!(gone.status, 404);
        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn guide_delete_refuses_another_users_row() {
        let (state, dir) = test_state();
        let now = crate::config::now_ms();
        state.pool.insert_guide(&crate::db::GuideRecord {
            slug: "owned".into(),
            title: "Owned".into(),
            author: None,
            date: None,
            ...
            created_by: Some("someone-else".into()),
            created_at: now,
            updated_at: now,
        }).unwrap();
        let res = handle(&state, Request::for_test("DELETE", "/api/guides/owned"));
        assert_eq!(res.status, 404);
        assert!(state.pool.get_guide("owned").unwrap().is_some());
        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn literal_guide_suffix_is_not_a_missing_api_route() {
        let (state, dir) = test_state();
        let res = handle(&state, Request::for_test("POST", "/api/guides/missing-guide/chapter-timing"));
        assert_eq!(res.status, 404);
        assert_eq!(json_body(&res).get_str("error"), Some("Guide not found"));
        let mut tts_req = Request::for_test("POST", "/api/tts");
        tts_req.set_test_body(b"{}".to_vec());
        let tts = handle(&state, tts_req);
        assert_eq!(tts.status, 400);
        assert_eq!(json_body(&tts).get_str("error"), Some("Missing 'text'"));
        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn guide_get_merges_streaming_timing_sidecar() {
        let (state, dir) = test_state();
        let now = crate::config::now_ms();
        state
            .pool
            .insert_guide(&crate::db::GuideRecord {
                slug: "live".into(),
                title: "Live".into(),
                author: None,
                date: None,
                ...
                timing_offset: 0.0,
                default_view_mode: "generated".into(),
                transcript: Some("original".into()),
                summary: None,
                source_url: None,
                jobs_json: Some(
                    crate::json::stringify(&crate::json::obj([(
                        "tts",
                        crate::json::obj([("status", crate::json::s("running"))]),
                    )])),
                ),
                chapters_json: "[]".into(),
                timing_json: None,
        ...
        std::fs::write(
            parts.join("timing.json"),
            br#"{"words":[{"w":"hi","t":0.1}],"transcript":"hi world"}"#,
        )
        .unwrap();
        let one = handle(&state, Request::for_test("GET", "/api/guides/live"));
        assert_eq!(one.status, 200);
        let body = json_body(&one);
        assert_eq!(body.get_str("transcript"), Some("hi world"));
        assert_eq!(
            body.get("timing")
    ...
    #[test]
    fn audio_range_is_served_from_public() {
        let (state, dir) = test_state();
        let audio = state.backend_dir.join("public").join("audio");
        std::fs::create_dir_all(&audio).unwrap();
        std::fs::write(audio.join("demo.mp3"), b"0123456789").unwrap();
        let mut req = Request::for_test("GET", "/audio/demo.mp3");
        req.set_test_header("range", "bytes=0-3");
        let res = handle(&state, req);
        assert_eq!(res.status, 206);
        assert_eq!(res.body, b"0123");
        let stream = handle(&state, Request::for_test("GET", "/api/guides/demo/stream.mp3"));
        assert_eq!(stream.status, 302);
        let parts = audio.join("live.parts");
        std::fs::create_dir_all(&parts).unwrap();
        std::fs::write(parts.join("state.json"), br#"{"chunksTotal":1,"chunksDone":0,"done":false,"failed":false}"#).unwrap();
        let live = handle(&state, Request::for_test("GET", "/api/guides/live/stream.mp3"));
        assert_eq!(live.status, 200);
        assert!(live.growing_file.is_some());
        assert_eq!(
            live.headers.iter().find(|(k, _)| k.eq_ignore_ascii_case("content-type")).map(|(_, v)| v.as_str()),
            Some("audio/mpeg")
        );
        std::fs::remove_dir_all(&dir).ok();
    }
