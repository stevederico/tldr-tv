...
//! compression, no `Upgrade`/WebSocket, and no trailer exposure (trailers are
//! read and discarded). Handlers normally return a complete body; one special
//! path ([`GrowingFileBody`]) tails a growing on-disk file with chunked
//! transfer encoding so progressive TTS can start before the render finishes.
...
/// Live state of a growing on-disk file being tailed with chunked transfer.
///
/// Matches the `state.json` shape written by the TTS render helper.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct GrowingFileState {
    /// All bytes have been written and the file will not grow further.
    pub done: bool,
    /// The producer failed; clients should stop waiting.
    pub failed: bool,
}

/// Action for one poll of a growing-file tail loop.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum GrowingFileAction {
    /// Send newly available bytes.
    Write,
    /// Close the response.
    End,
...
/// Pure state-machine step for a growing-file response (matches Node
/// `__testStreamTailNext`).
///
/// # Arguments
/// * `offset` — bytes already written to the client
/// * `size` — current on-disk file size
/// * `state` — parsed producer state, or `None` when the state file is gone
pub fn growing_file_next(
    offset: u64,
    size: u64,
    state: Option<GrowingFileState>,
) -> GrowingFileAction {
    if size > offset {
        return GrowingFileAction::Write;
    }
    match state {
        None => GrowingFileAction::End,
        Some(s) if s.failed => GrowingFileAction::End,
        Some(s) if s.done && offset >= size => GrowingFileAction::End,
        Some(_) => GrowingFileAction::Wait,
    }
}

/// Tail a growing file with `Transfer-Encoding: chunked` until the producer
/// marks done/failed (or the idle timeout fires).
#[derive(Debug, Clone)]
pub struct GrowingFileBody {
    /// Path of the file that grows as the producer writes.
    pub path: PathBuf,
    /// Sidecar JSON with `{ done, failed }` (other fields ignored).
    pub state_path: PathBuf,
    /// Sleep between polls when waiting for more bytes.
    pub poll_ms: u64,
    /// Give up after this much consecutive idle time with nothing to write.
    pub max_idle_ms: u64,
}

/// A complete HTTP response produced by a handler.
///
/// `Date`, `Connection` and framing (`Content-Length` or `Transfer-Encoding`)
/// are written by the server; any copies a handler puts in `headers` are
/// dropped so the wire framing stays authoritative.
#[derive(Debug, Clone)]
pub struct Response {
    /// HTTP status code.
    pub status: u16,
    ...
    /// Response body. Empty for bodyless statuses and for growing-file tails.
    pub body: Vec<u8>,
    /// When set, the server streams this file with chunked transfer encoding
    /// instead of sending [`Self::body`].
    pub growing_file: Option<GrowingFileBody>,
}
...
impl Response {
    ...
    /// Stream a growing on-disk file with `Transfer-Encoding: chunked`.
    pub fn growing_file(status: u16, content_type: &str, body: GrowingFileBody) -> Self {
        Response {
            status,
            headers: vec![("Content-Type".to_string(), content_type.to_string())],
            body: Vec::new(),
            growing_file: Some(body),
        }
    }
...
///
/// `Date` and `Connection` are always sent. Buffered bodies get
/// `Content-Length` (including `HEAD`, where the length is computed as if the
/// method were `GET` but no body follows — RFC 7231 §4.3.2). Growing-file
/// bodies use `Transfer-Encoding: chunked` on HTTP/1.1 (and an unframed body
/// with `Connection: close` on HTTP/1.0). 1xx/204/304 carry neither body nor
/// length (RFC 7230 §3.3.2).
fn write_response(
    w: &mut TcpStream,
    res: &Response,
    head_only: bool,
    ...
    if let Some(grow) = &res.growing_file {
        return write_growing_file(w, res, grow, head_only, keep_alive, http_11);
    }
...
/// Write headers, then tail `grow.path` until the producer finishes.
fn write_growing_file(
    w: &mut TcpStream,
    res: &Response,
    grow: &GrowingFileBody,
    head_only: bool,
    keep_alive: bool,
    http_11: bool,
) -> std::io::Result<()> {
    // Chunked framing needs HTTP/1.1; otherwise close after the body.
    let use_chunked = http_11;
    let keep = keep_alive && use_chunked;
    let version = if http_11 { "HTTP/1.1" } else { "HTTP/1.0" };
    let mut out: Vec<u8> = Vec::with_capacity(256);
    out.extend_from_slice(
        format!("{} {} {}\r\n", version, res.status, reason_phrase(res.status)).as_bytes(),
    );
    out.extend_from_slice(format!("Date: {}\r\n", http_date(SystemTime::now())).as_bytes());
    out.extend_from_slice(if keep {
        b"Connection: keep-alive\r\n".as_slice()
    } else {
        b"Connection: close\r\n".as_slice()
    });
    ...
    loop {
        let size = file_size(&grow.path);
        let state = read_growing_state(&grow.state_path);
        match growing_file_next(offset, size, state) {
            GrowingFileAction::Write => {
                let n = (size - offset) as usize;
                let bytes = read_file_range(&grow.path, offset, n)?;
                if use_chunked {
                    write_chunk(w, &bytes)?;
                } else {
                    w.write_all(&bytes)?;
            ...
            GrowingFileAction::End => break,
            GrowingFileAction::Wait => {
                if idle_ms >= grow.max_idle_ms {
                    break;
                }
                thread::sleep(Duration::from_millis(grow.poll_ms));
                idle_ms = idle_ms.saturating_add(grow.poll_ms);
            }
        }
    }
    if use_chunked {
...
fn read_file_range(path: &Path, start: u64, len: usize) -> std::io::Result<Vec<u8>> {
...
fn read_growing_state(path: &Path) -> Option<GrowingFileState> {
    let raw = fs::read_to_string(path).ok()?;
    let parsed = crate::json::parse(raw.as_bytes()).ok()?;
    Some(GrowingFileState {
        done: parsed.get("done").and_then(crate::json::Json::as_bool) == Some(true),
        failed: parsed.get("failed").and_then(crate::json::Json::as_bool) == Some(true),
    })
}
...
mod tests {
    ...
    #[test]
    fn growing_file_next_matches_stream_tail_machine() {
        assert_eq!(
            growing_file_next(0, 10, Some(GrowingFileState { done: false, failed: false })),
            GrowingFileAction::Write
        );
        assert_eq!(
            growing_file_next(10, 10, Some(GrowingFileState { done: false, failed: false })),
            GrowingFileAction::Wait
        );
        assert_eq!(
            growing_file_next(10, 10, Some(GrowingFileState { done: true, failed: false })),
            GrowingFileAction::End
        );
        assert_eq!(
            growing_file_next(5, 5, Some(GrowingFileState { done: false, failed: true })),
            GrowingFileAction::End
        );
        assert_eq!(growing_file_next(0, 0, None), GrowingFileAction::End);
    }
